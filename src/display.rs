...
pub fn render(game: &Game) -> Result<()> {
    ...
    let board_width_start = terminal_width / 2 - WIDTH / 2;
    let board_width_end = board_width_start + WIDTH;

    let board_height_start = 1 as u16;
    let board_height_end = HEIGHT - 1;
    ...
    for x in board_width_start + 1..board_width_end - 1 {
        for y in board_height_start..board_height_end {
            stdout
    ...
    for (i, tetromino) in game.next.iter().enumerate() {
        ...
        for position in tetromino.shape.iter().map(|(x, y)| (*x as u16, *y as u16)) {
            stdout
                .queue(MoveTo((position.0 - 3) * 2 + board_width_end + 2, HEIGHT - position.1 + 1 + (i as u16 * 3)))?
                .queue(PrintStyledContent(" ".on(tetromino.color)))?
                .queue(MoveTo((position.0 - 3) * 2 + board_width_end + 1, HEIGHT - position.1 + 1 + (i as u16 * 3)))?
                .queue(PrintStyledContent(" ".on(tetromino.color)))?;
        }
    }
    if let Some(holding) = &game.holding {
        ...
        for position in holding.shape.iter().map(|(x, y)| (*x as u16, *y as u16)) {
            stdout
                .queue(MoveTo((position.0 - 3) * 2 + board_width_start - 9, HEIGHT - position.1 + 1))?
                .queue(PrintStyledContent(" ".on(holding.color)))?
                .queue(MoveTo((position.0 - 3) * 2 + board_width_start - 8, HEIGHT - position.1 + 1))?
                .queue(PrintStyledContent(" ".on(holding.color)))?;
        }
    }
    stdout
            ...
            stdout
                ...
                .queue(Print(
                    if x == board_width_start && y == 0 {
                        "╔"
                    } else if x == board_width_start && y == HEIGHT - 1 {
                        "╚"
                    } else if x == board_width_end - 1 && y == 0 {
                        "╗"
                    } else if x == board_width_end - 1 && y == HEIGHT - 1 {
                        "╝"
