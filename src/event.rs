    ...
    Ok(match event {
        Event::Key(KeyEvent { kind, code, .. }) => {
            if kind == KeyEventKind::Press {
                match code {
                    _ if config::controls::ROTATE_RIGHT.contains(&code) => {
                        game.rotate(RotationDirection::Clockwise);
                        reset_lock_timer(&game, lock_delay);
                    },
                    _ if config::controls::MOVE_LEFT.contains(&code) => {
                        game.shift(ShiftDirection::Left);
                        if !game.hitting_left(&game.falling) {
                            reset_lock_timer(&game, lock_delay);
                        }
                    },
                    _ if config::controls::SOFT_DROP.contains(&code) => {
                        if !game.hitting_bottom(&game.falling) {
                            lock_delay.as_mut().reset(Instant::now() + LOCK_DURATION);
                        }
                        game.soft_drop();
                    },
                    _ if config::controls::MOVE_RIGHT.contains(&code) => {
                        game.shift(ShiftDirection::Right);
                        if !game.hitting_right(&game.falling) {
                            reset_lock_timer(&game, lock_delay);
                        }
                    },
                    _ if config::controls::HARD_DROP.contains(&code) => {
                        game.hard_drop();
                    },
                    _ if config::controls::ROTATE_LEFT.contains(&code) => {
